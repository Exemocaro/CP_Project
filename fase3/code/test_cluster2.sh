...
processes=(1 2 4 8 10 12 16 20 24 28 32 36 40)
thread=40
...
do
    ...
    export OMP_NUM_THREADS=${thread}; time mpirun -np ${nprocs} ./MDparmpi.exe < inputdata.txt > output/output_${nprocs}.txt
    # time mpirun -np ${nprocs} ./MDmpi.exe < inputdata.txt > output/output_${nprocs}.txt
